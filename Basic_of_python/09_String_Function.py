...
A="Abhishek"
print(len(A))#--------------------------IMP Term Realted to the length.
...
|'__' in "---"        | Checks if substring is present                 |   'th' in "python" → True            |
...
| Method                            | Description             | Example                                           |
| --------------------------------- | ------------------------|------------------------------------------------- |
|__.replace("old","new")            | Replaces substring      |  "goodbye".replace("good", "hello") → 'hellobye'  |
|__.split('separator')              | Splits string into list |  "a,b,c".split(',') → ['a', 'b', 'c']             |
|"saparator".join(['list','_','..'])| Joins list into string  |  ",".join(['a', 'b']) → 'a,b'                     |
